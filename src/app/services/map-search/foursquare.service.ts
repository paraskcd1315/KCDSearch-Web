...
import { FOURSQUARE_API_URL_SEARCH, GEO_LOCATION_PRECISION } from '../../utils/constants.utils';
import { GeolocationService } from './geolocation.service';
import { MapCacheService } from '../cache/map-cache.service';
import { HttpClient } from '@angular/common/http';
import { firstValueFrom } from 'rxjs';
import { roundToGeoLocationPrecision } from '../../utils/map.utils';
...
export class FoursquareService {
  ...
  async search(query: string): Promise<FoursquarePlaceResult[]> {
    ...
    if (Array.isArray(cached)) {
      return cached as FoursquarePlaceResult[];
    }
  ...
  private cacheKey(lat: number, lon: number, query: string): string {
    const roundedLat = roundToGeoLocationPrecision(lat, GEO_LOCATION_PRECISION);
    const roundedLon = roundToGeoLocationPrecision(lon, GEO_LOCATION_PRECISION);

    return `${roundedLat}|${roundedLon}|${query.trim().toLowerCase()}`;
  }
  ...
  private async fetchPlaces(
    ...
    if (version) {
      headers['X-Places-Api-Version'] = version;
    }
